...
const width = (canvas.width = window.innerWidth - Math.floor(window.innerWidth / 10));
const height = (canvas.height = window.innerHeight - Math.floor(window.innerHeight / 10));
...
function draw () {
    if (alive) {
        ...
        if (rand(1, obstacleGeneration) === 1) {
            ...
            let newCircleRadius = newCircleEatble ? rand(1, playerRadius) : rand(playerRadius + 10, playerRadius + 50);
            ...
            let newCircle = new Circle(newCircleX, newCircleY, newCircleColor, newCircleRadius, newCircleXVelocity, newCircleYVelocity);
            drawnCircles.push(newCircle);

            ctx.fillStyle = newCircle.color;
            ctx.beginPath();
            ctx.arc(newCircle.x, newCircle.y, newCircle.radius, degToRad(0), degToRad(360));
            ctx.fill();
        }
        ...
        for (let circle of drawnCircles) {
            ...
            if (Math.sqrt(dx * dx + dy * dy) < playerRadius + circle.radius) { // touching
            ...
            if (circle.x < 0 - offscreenGracePixels || circle.x > width + offscreenGracePixels || circle.y < 0 - offscreenGracePixels || circle.y > height + offscreenGracePixels) {
                circle.queuedForRemoval = true;
            }
        ...
        drawnCircles = drawnCircles.filter((value) => {return !value.queuedForRemoval});
        ...
        if (!(playerScore % 10) && playerScore !== 0 && !obstacleGenerationUpdated) {
...
function die () {
    ...
    // Save high scores
    if ((playerAscensions > highAscensions) || (playerAscensions == highAscensions && playerScore > highScore)) {
...
function drawLastMessage (isNewHighScore) {
    ...
    ctx.fillText("Click to restart", width / 2 - clickToRestartWidth / 2, height / 2  + 60);
}

function restart() { // Can also be used as first start
    drawBackground();
    if (alive) {
